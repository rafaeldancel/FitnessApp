    ...
    switch (step) {
      ...
      case 5: // Health Restrictions
        if (formState.hasHealthRestrictions && formState.healthRestrictions.length === 0) {
          newErrors.healthRestrictions = 'Please select at least one health restriction or toggle off'
        }
        ...
        if (otherRestriction && (!otherRestriction.description || otherRestriction.description.trim().length < 10)) {
          newErrors.healthRestrictions = 'Please provide a description for "Other" (minimum 10 characters)'
        }
        break
